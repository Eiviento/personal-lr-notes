...
8. [UTF-8 与 GBK，以及 JSON 的编码规则](#8-utf-8-与-gbk以及-json-的编码规则)
9. [为什么必须 base64：二进制被文本协议「误伤」的例子](#9-为什么必须-base64二进制被文本协议误伤的例子)
10. [为什么说 JS 难解析表单](#10-为什么说-js-难解析表单)
11. 全景速查与面试要点
...
## 8. UTF-8 与 GBK，以及 JSON 的编码规则

前一节说「JSON 规定用 UTF-8」。这里把 UTF-8、GBK 和 JSON 的编码规则讲透。

### 8.1 两个层次：字符集 vs 字符编码

- **字符集（charset / repertoire）**：一张「字符 ↔ 编号」的对照表。Unicode 规定「中」的编号是 `U+4E2D`。它只定义有哪些字符、各叫什么号，**不定义怎么存成字节**。
- **字符编码（encoding）**：把编号**变成字节序列**的规则。同一个编号可以有多种编码方式。

关键：**UTF-8 是 Unicode 这个字符集的一种编码方式；而 GBK 是「字符集 + 编码」绑在一起的中国标准。**

### 8.2 UTF-8

Unicode 的**变长编码**，1～4 字节，长度由首字节高位模式决定：

| 字节数 | 首字节模式 | 码点范围 | 典型内容 |
|---|---|---|---|
| 1 | `0xxxxxxx` | U+0000–U+007F | ASCII（英文、数字、标点） |
| 2 | `110xxxxx 10xxxxxx` | U+0080–U+07FF | 拉丁扩展、希腊、西里尔 |
| 3 | `1110xxxx 10xxxxxx 10xxxxxx` | U+0800–U+FFFF | **常用汉字**、日文、韩文 |
| 4 | `11110xxx 10xxxxxx ...` | U+10000–U+10FFFF | emoji、生僻字 |

三个设计优点：**① ASCII 完全兼容**（`A` 还是 `0x41`，旧英文系统不改造也不坏）；**② 自同步**（续字节永远 `10xxxxxx` 开头，坏一个字节不污染后续）；**③ 无字节序问题**。唯一代价：中文比 GBK 多占 1 字节。

### 8.3 GBK

全称「汉字内码扩展规范」，1995 年发布，GB2312 的扩展。变长 1–2 字节：单字节 `0x00–0x7F` 等同 ASCII；双字节首字节 `0x81–0xFE`、第二字节 `0x40–0xFE`（跳过 `0x7F`）。

它只覆盖中文及少量相关文字，**表示不了 emoji、阿拉伯文等**。Windows 简体中文系统里记事本「ANSI」保存格式就是它（代码页 936 / CP936）。

> 家族关系：**GB2312（1980，6763 汉字）⊂ GBK（1995，2 万多汉字）⊂ GB18030（覆盖全部 Unicode，中国强制国标，用 1/2/4 字节）**。

### 8.4 实测对照

| 字符 | 码点 | UTF-8 字节（长度） | GBK 字节（长度） |
|---|---|---|---|
| `A` | U+0041 | `41`（1B） | `41`（1B） |
| `中` | U+4E2D | `e4 b8 ad`（3B） | `d6 d0`（2B） |
| `你` | U+4F60 | `e4 bd a0`（3B） | `c4 e3`（2B） |
| `好` | U+597D | `e5 a5 bd`（3B） | `ba c3`（2B） |
| `😀` | U+1F600 | `f0 9f 98 80`（4B） | **无法编码 ✗** |

ASCII 段两表完全一致（这就是兼容的含义）；汉字 UTF-8 多 1 字节；emoji 是 GBK 表外之物——**这就是 GBK 的死穴**。

### 8.5 乱码是怎么产生的

原理一句话：**编码用一张表、解码用了另一张表，字节被翻译成了错的字符。** 三组经典现象（实测复现）：

- **① GBK 字节被当 UTF-8 解**：`你好` 的 GBK 字节 `c4 e3 ba c3` → 全不是合法序列，变成 `���`（替换符）。
- **② UTF-8 字节被当 GBK 解**：`你好` 的 UTF-8 字节 `e4 bd a0 e5 a5 bd` → **`浣犲ソ`**。因为每两字节恰好凑成合法 GBK 汉字，于是**不报错、但内容全错**——比 `�` 更迷惑。
- **③「锟斤拷」「烫烫烫」的来历**：
  - 「锟斤拷」来自 UTF-8 替换符 `U+FFFD`（字节 `EF BF BD`）。数据经一次错误解码产生 � 后，`EF BF BD EF BF BD…` 再被 GBK 解 → 「锟斤拷」。
  - 「烫烫烫」「屯屯屯」来自未初始化内存：MSVC 调试模式用 `0xCC` 填栈、`0xCD` 填堆，`CC CC` 在 GBK 里正是「烫」。

### 8.6 JSON 的字符编码规则

和 HTML 不同，**JSON 不能指定字符编码**：

| | 能否指定字符编码 |
|---|---|
| HTML | **能**——`Content-Type: text/html; charset=gbk` |
| JSON | **不能**——规范锁死 UTF-8 |

RFC 8259 规定：

- §8.1：「JSON text exchanged between systems that are not part of a closed ecosystem **MUST be encoded using UTF-8**.」
- IANA 媒体类型注册：「**No 'charset' parameter is defined for this registration. Adding one has no effect on compliant recipients.**」——所以 `application/json; charset=gbk` 里的参数会被**忽略**。
- **禁止 BOM**：「Implementations **MUST NOT add a byte order mark** to the beginning of a networked-transmitted JSON text.」（别给它加 `EF BB BF`，那是给 CSV/Excel 用的招，用在 JSON 上有害。）
- 历史：更早的 RFC 4627 曾允许 UTF-8/16/32 并用 BOM 探测；7159→8259 收紧为「交换时必须 UTF-8」。

设计意图：JSON 是**数据交换格式**，钉死 UTF-8 才能让接收方永远不用猜编码。内容层面则自由——任何字符都能用 `\uXXXX` 转义表达，emoji 用**代理对**：

```python
{"msg": "\u4f60\u597d\ud83d\ude00"}   # ensure_ascii=True → 纯 ASCII
{"msg": "你好😀"}                      # 无论哪种写法，最终字节都是 UTF-8
```

### 8.7 实践中的坑

- **HTTP/HTML**：编码判定优先级 **BOM > HTTP 头 charset > `<meta charset>`**，三者打架就乱码。
- **Windows 记事本**：默认存「ANSI」= GBK，不是 UTF-8；跨平台常出问题。
- **Excel 打开 CSV 乱码**：无 BOM 的 UTF-8 被 Excel 按本地代码页（GBK）解；加 UTF-8 BOM 可让它认出来。
- **数据库/源码**：连接字符集、源文件编码不一致都会乱。
- **结论**：新项目、新文件、跨平台传输**一律 UTF-8（无 BOM）**；GBK 只作「读老系统 / 老数据」的兼容存在。

---

## 9. 为什么必须 base64：二进制被文本协议「误伤」的例子
...
### 10.3 为什么 multipart 本身比 JSON 难解析

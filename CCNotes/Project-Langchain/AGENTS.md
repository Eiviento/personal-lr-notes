...
## Stack
- Language: Python 3.10（conda env `agent_env`，解释器 E:/software/OfficeWorkLife/Anaconda/envs/agent_env/python.exe）
- Framework: LangGraph 1.2.9 + langchain-core 1.4.9 + langchain-openai（DeepSeek API，OpenAI 兼容）
- Run: `PYTHONIOENCODING=utf-8 <agent_env python> scripts/<script>.py`（Windows GBK 坑：必须 UTF-8 前缀）
- Test: 零成本冒烟（假 agent 不调 API）+ 实跑证据留档 outputs\

## Conventions
- 教学项目：每 Phase = 代码 + 实跑证据 + lessons\ 落盘（零基础可读：做什么/为什么/不做会怎样）；讲解带执行示例、一次只问一个问题、代码精读用三问法
- 证据纪律：声称跑通须 exit 0 + 日志；引用实录写"单次实跑、未挑选"；报告数字来自本轮工具输出不凭记忆
- .env（gitignore）存 DEEPSEEK_API_KEY；脚本用 python-dotenv 或 os.getenv 加载，禁止读/提交 .env 内容
- API 锚点以 agent_env 实测为准（langgraph 1.2.9 部分 API 在迁移期），落教学代码前先跑最小 import 探针
- 提交经 deliver_task 归属提交（git 仓库根在上级 personal-lr-notes，本目录以 CCNotes\Project-Langchain\ 入库）

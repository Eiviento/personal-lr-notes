...
> 更新时间：2026-08-02（第十一会话）
> 学习进度：全部 7 阶段 36 知识点已完成 ✅ + 附录报文手册已完成 ✅ + HTML 交互页面已完成 ✅
> 本会话重点：**加密生命周期深度补充**——加密使用场景、全加密开关、暂停恢复机制、状态机
...
## 本会话（第十一会话）：加密生命周期深度补充

### 做了什么

用户在学习 SMP 和链路层加密的基础上，追问了三个递进问题：

1. **SMP 需要搭配 LL Control PDU 使用吗？** → 是的。SMP 管密钥生成，LL Control PDU 管加密启动。SMP 先跑算出 LTK，LL_ENC_REQ/RSP → LL_START_ENC_REQ/RSP 接力执行"从现在开始加密"。

2. **什么场景用加密？资源消耗大吗？** → 加密由 GATT 权限自动触发（不是 App 手动调），7 种典型场景（医疗/支付/门锁/OTA/工业传感器/个人设备/HID）。资源消耗几乎为零——BLE Controller 强制内置 AES 硬件加速器，功耗不到射频发射的 0.3%。

3. **开了加密后所有数据都加密吗？能关吗？** → 全部加密，包括空包（不带 MIC 的空包 = DoS 攻击面）。有 LL_PAUSE_ENC 暂停机制，但只能"暂停→换钥匙→恢复"，不能永久关掉。退出加密的唯一方式是断开连接。

### 本次会话产出

| 类型 | 文件 | 变更 |
|------|------|------|
| 笔记 | `5.6-链路层加密.md` | **新增 Q3（Q3a~Q3d）**：使用场景表、全加密开关说明、暂停恢复机制、生命周期状态机 |
| HTML | `ble-protocol-manual.html` | **新增 §9.7**（9.7a~9.7d）+ 导航栏子项 + 附录"OPPO Enco Air2"→"蓝牙耳机" |
| 交接 | `HANDOFF.md` | **更新**：补充本会话内容 |

### 本会话建立的深层理解

1. **SMP ⇄ LL Control PDU 接力**：SMP = 密钥管理局，LL Control PDU = 门卫。两套协议分属不同层但必须协作
2. **加密是链路层总开关**：一开全加密，空包都不放过
3. **有暂停，没有永久停**：LL_PAUSE_ENC 只用于密钥轮换，不存在 LL_STOP_ENC
4. **加密开销几乎为零**：AES-CCM 硬件加速强制内置，"省电关加密"不成立
5. **GATT 权限是加密触发器**：App 正常读写，协议栈发现权限不够时自动触发加密，对 App 透明
...
| 🔐 加密生命周期 §9.7 | ★ 本会话新增：使用场景表（7种）、全加密开关机制（空包也必须加密的原因）、暂停恢复流程（LL_PAUSE_ENC → 密钥轮换 → 重新加密）、加密状态机图 |
...
- **SMP 和 LL Control PDU 是协作关系，不是替代关系**：SMP 生成 LTK，LL_ENC_REQ/RSP 用 LTK 派生 SessionKey，LL_START_ENC_REQ/RSP 切换加密模式。不能跳过任何一环。
- **加密是链路层总开关，一开全加密**：不是"只加密需要保护的包"——ATT Write 加密，ATT Read 也加密，空包照样加密。加密由 LL Header 的 LLID 判断 Data vs Control（Control PDU 始终明文），不由上层数据敏感度决定。
- **空包也带 MIC**：Payload 长度可以为 0，但 MIC 不能缺——否则攻击者可以插入不带 MIC 的伪造空包来扰乱 SN/NESN 序列号导致 DoS。
- **LL_PAUSE_ENC 不是"省电暂停"，是"换钥匙"**：暂停只能用于密钥更新（安全升级/密钥轮换），不能"暂停→就这样明文跑下去"。不存在 LL_STOP_ENC。
- **加密触发权在 GATT 权限表**：App 不手动调加密。设备重新建连后，如果 App 访问需要加密的 Characteristic，协议栈自动触发加密。不访问就不会触发。
- **AES-CCM 硬件加速是 BLE 规范强制要求**：不是可选功能，每个 BLE Controller 硅片上都有 AES 引擎。"为省电关加密"在工程上不成立——硅片面积已经花出去了。
- **LL_START_ENC_REQ 本身是明文**：这个容易想当然写成加密后发送，但逻辑上对方必须先读到"即将切换到加密"才能切换解密引擎。在你笔记里已有（5.6 §四），但教学时学生容易问。
...
*交接时间：2026-08-02（第十一会话：加密生命周期深度补充）*

...
问："Function Calling 和 MCP 是不是同一个东西？前者只能代码内部用，后者可以部署在单独服务上调用？"

答：对了一大半，但要拧准两点：

1. **不是同一个东西，是两个层面 + 协作关系**。Function Calling 是"模型 API 的能力"（解决**模型怎么用工具**）；MCP 是"传输协议 + 部署标准"（解决**工具怎么接入、住在哪里**）。MCP 服务器上的工具最终被模型调用时，用的**还是** Function Calling 机制——模型照样点菜。两者是接力，不是替代。
2. **MCP 解决的问题不只是部署位置**。部署独立性（stdio 子进程 / HTTP 远程服务）只是其一；更重要的是 **N×M 复用**：写一次，任何 MCP 客户端都能插（纯 Function Calling 换个应用就要搬代码重绑定）。另外 MCP 多一个用法：**没有模型也能调工具**（客户端直接 call_tool）——Function Calling 是"模型专用通道"，MCP 是"通用插头"，代码和模型都能插。

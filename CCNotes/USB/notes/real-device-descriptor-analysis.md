...
> 所有原始数据来自 `usb设备1的描述符.txt` / `usb设备2的描述符.txt` / `usb设备3的描述符.txt`
...
> 格式：字段 = 值（十六进制），去掉了抓取工具的行号与逐位展开，保留全部描述符字段。来源 `usb设备1的描述符.txt`（序列号 G11376317）。
...
> 来源 `usb设备2的描述符.txt`（序列号 E83518457）。与设备 1 完全相同的部分用「同设备 1」省略，仅列出差异与新增段。
...
> 来源 `usb设备3的描述符.txt`。设备 3 没有原始 USB 描述符，以下为 Windows 驱动解析后的 KS 层数据。
